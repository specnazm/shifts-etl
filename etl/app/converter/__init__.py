...
class ShiftConverter(Converter):
    def __init__(self, spark):
        Converter.__init__(self, spark, name='shifts', columns=['shift_id', 'shift_date', 'shift_start', 'shift_finish'])
        self.schema = shift_schema

    def create_df(self, data):
        df = self.spark.createDataFrame(self.spark.sparkContext.parallelize(data), schema=self.schema) \
    ...
    def __rename_attributes(self, df):
        renamed_df = df.withColumnRenamed("date", "shift_date") \
    ...
    def __convert_date_attr(self, df):
        df = df.withColumn('shift_date', to_date(col('shift_date'), 'yyyy-MM-dd')) \
...
class BreakConverter(Converter):
    def __init__(self, spark):
        Converter.__init__(self, spark, name='breaks', columns= ['break_id', 'break_start', 'break_finish', 'is_paid', 'shift_id'])
        self.schema = break_schema

    def create_df(self, data):
        breaks_df = data.withColumn("break_obj", explode("breaks"))
        breaks_df = breaks_df.withColumn('break_id', breaks_df['break_obj'].id) \
    ...
    def __convert_date_attr(self, df):
        df = df.withColumn('break_start', to_timestamp(col('break_start') / 1e3)) \
...
class AllowanceConverter(Converter):
    def __init__(self, spark):
        Converter.__init__(self, spark, name='allowances', columns=['allowance_id', 'allowance_value', 'allowance_cost', 'shift_id'])
        self.schema = allowance_schema

    def create_df(self, data):
        df = data.withColumn("allow_obj", explode("allowances"))
        df = df.withColumn('allowance_id', df['allow_obj'].id) \
...
class AwardConverter(Converter):
    def __init__(self, spark):
        Converter.__init__(self, spark, name='award_interpretations', columns=['award_id', 'award_date', 'award_units', 'award_cost', 'shift_id'])
        self.schema = award_schema

    def create_df(self, data):
        df = data.withColumn("award_obj", explode("award_interpretations"))
        df = df.withColumn('award_id', df['award_obj'].id) \
