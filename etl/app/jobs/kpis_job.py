...
def calculate_kpis_job():
    """
    Generates dataframe with all transformations.
    :return: None
    """
    results = []
...
def create_kpi_df(results):
    """
    Creates dataframe from results.
    :param results: Data collection after all applied transformations
    """
    for result in results:
        # Some KPI values are stored as floats, some as ints, so casting is needed here
...
def mean_break_len(break_df):
    """
    Mean break length calculation.
    :param break_df: Collection of breaks presented as Spark DataFrame
    """
    log.info("Started task : MEAN BREAK TIME")

    if (not break_df):
        return
    break_df = break_df.withColumn('duration_min',
                                   round((col("break_finish").cast("long") - col('break_start').cast("long")) / 60))
...
def min_shift_len(shifts_df):
    """
    Minimal shift length calculation
    :param shifts_df: Collection of shifts presented as Spark DataFrame
    """
    log.info("Started task : MIN SHIFT LEN")
    if (not shifts_df):
        return
    shifts_df = shifts_df.withColumn('len_hours',
                                     round((col("shift_finish").cast("long") - col('shift_start').cast("long")) / 3600))
...
def max_allowance_cost_14d(shifts_df):
    """
    Maximum allowance cost in the previous 14 days
    :param shifts_df: Collection of shifts presented as Spark DataFrame
    """
...
def max_break_free_period(break_df, shifts_df):
    """
    Longest period in days without breaks
    :param break_df: Collection of breaks presented as Spark DataFrame
    :param shifts_df: Collection of shifts presented as Spark DataFrame
    """
